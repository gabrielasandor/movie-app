  ...
  return (
    <div className="root">
      ...
      <div className="movie_content">
        {movies.length > 0 && movies.map((movie) => <Link to={`/MovieId/${movie.id}`}> <Movie key={movie.id} {...movie} /> </Link>
        )}
      </div>
    </div>
  )
...
function MovieDescription() {
  ...
  useEffect(() => {
    fetch(`${url}/movie/${MovieId}?api_key=${movieKey}&language=en-US`)
      .then((res) => res.json())
      .then((data) => {
        setMovie(data);
      });
  ...
  if (movie == null) {
    ...
    return <div >
      <div className="mainperson">
        <div className="movie" ><img src={`https://image.tmdb.org/t/p/original${movie.poster_path} `}></img>  </div>
        <div className="pers_info">
          <h1>{movie.title}</h1>
          <h2> Release date: {movie.release_date}</h2>
          <h2> Duration: {movie.runtime}min</h2>
...
<div className="movie_content">
      {upcoming.map((movie) => <Link to={`/MovieId/${movie.id}`}> <Movie key={movie.id} {...movie} /> </Link>
      )}
    </div>
  </div>
  )
